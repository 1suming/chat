...
static void serv_msg(void *fd)
{
	...
	//notice othe client who is exiting
	notice.Version=1;
	...
	while((n=Read(sockfd,buf,MAX_NUM_STR))>0)
	{
		...
		if(msg.MsgType==MSG_LOGIN)
		{
            ...
            Write(sockfd,buf,sizeof(ChatSysPdu));
		...
		else if(msg.MsgType==MSG_CHAT)
		{
			...
			for(tmp=head;tmp!=NULL&&tmp->flag!=INVALID;tmp=tmp->next)
            {
                if(tmp->fd!=sockfd)
                {
                    Write(tmp->fd,buf,sizeof(ChatSysPdu));
                }
            }
            pthread_mutex_unlock(&mutex);
	    }
...
int main()
{
	...
	while(1)
	{
		...
		//create thread
		pthread_create(&tid,NULL,(void *)serv_msg,(void *)&connfd);
